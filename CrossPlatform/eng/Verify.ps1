...
if ($manifestCaseCount -ne 44) {
    Stop-Gate -Message "Contract case manifest count differs from the reviewed baseline of 44: $manifestCaseCount" -Step 'contract'
}

$contractCounts = @{}
$contractPattern = '^PASS port-contract cases=(44) assertions=(522) failures=0 elapsed_ms=[0-9]+\s*$'
foreach ($configuration in @('Debug', 'Release')) {
    ...
    if ($caseCount -ne $manifestCaseCount -or $assertionCount -ne 522) {
        Stop-Gate -Message "Contract $configuration counts do not match the reviewed 44-case, 522-assertion baseline." -Output $result.StdOut -Step $step
    }
