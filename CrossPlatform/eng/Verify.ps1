...
if ($manifestCaseCount -ne 42) {
    Stop-Gate -Message "Contract case manifest count differs from the reviewed baseline of 42: $manifestCaseCount" -Step 'contract'
}

$contractCounts = @{}
$contractPattern = '^PASS port-contract cases=(42) assertions=(500) failures=0 elapsed_ms=[0-9]+\s*$'
foreach ($configuration in @('Debug', 'Release')) {
    ...
    if ($caseCount -ne $manifestCaseCount -or $assertionCount -ne 500) {
        Stop-Gate -Message "Contract $configuration counts do not match the reviewed 42-case, 500-assertion baseline." -Output $result.StdOut -Step $step
    }
