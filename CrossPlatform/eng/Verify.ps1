...
if ($manifestCaseCount -ne 46) {
    Stop-Gate -Message "Contract case manifest count differs from the reviewed baseline of 46: $manifestCaseCount" -Step 'contract'
}

$contractCounts = @{}
$contractPattern = '^PASS port-contract cases=(46) assertions=(545) failures=0 elapsed_ms=[0-9]+\s*$'
foreach ($configuration in @('Debug', 'Release')) {
    ...
    if ($caseCount -ne $manifestCaseCount -or $assertionCount -ne 545) {
        Stop-Gate -Message "Contract $configuration counts do not match the reviewed 46-case, 545-assertion baseline." -Output $result.StdOut -Step $step
    }
