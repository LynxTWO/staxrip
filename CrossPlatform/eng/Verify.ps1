...
if ($manifestCaseCount -ne 45) {
    Stop-Gate -Message "Contract case manifest count differs from the reviewed baseline of 45: $manifestCaseCount" -Step 'contract'
}

$contractCounts = @{}
$contractPattern = '^PASS port-contract cases=(45) assertions=(527) failures=0 elapsed_ms=[0-9]+\s*$'
foreach ($configuration in @('Debug', 'Release')) {
    ...
    if ($caseCount -ne $manifestCaseCount -or $assertionCount -ne 527) {
        Stop-Gate -Message "Contract $configuration counts do not match the reviewed 45-case, 527-assertion baseline." -Output $result.StdOut -Step $step
    }
