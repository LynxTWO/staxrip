...
if ($manifestCaseCount -ne 37) {
    Stop-Gate -Message "Contract case manifest count differs from the reviewed baseline of 37: $manifestCaseCount" -Step 'contract'
}

$contractCounts = @{}
$contractPattern = '^PASS port-contract cases=(37) assertions=(486) failures=0 elapsed_ms=[0-9]+\s*$'
foreach ($configuration in @('Debug', 'Release')) {
    ...
    if ($caseCount -ne $manifestCaseCount -or $assertionCount -ne 486) {
        Stop-Gate -Message "Contract $configuration counts do not match the reviewed 37-case, 486-assertion baseline." -Output $result.StdOut -Step $step
    }
