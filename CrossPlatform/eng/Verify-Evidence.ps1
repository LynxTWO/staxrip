...
try {
    ...
    $expectedCases = @(
        ...
        'CT-029', 'CT-030', 'CT-031', 'CT-032', 'CT-033',
    ...
    for ($index = 0; $index -lt $stepItems.Count; $index++) {
        ...
        if ($expectedStepId -in @('port-contract-debug', 'port-contract-release')) {
            ...
            Confirm-Check (
                (Get-JsonIntegerField -Fields $stepFields -Name 'cases' -Id 'summary-contract-cases') -eq 42) `
                'summary-contract-cases-value'
            Confirm-Check (
                (Get-JsonIntegerField -Fields $stepFields -Name 'assertions' -Id 'summary-contract-assertions') -eq 500) `
