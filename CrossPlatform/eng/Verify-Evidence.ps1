...
try {
    ...
    $expectedCases = @(
        ...
        'CT-029', 'CT-030', 'CT-031', 'CT-032', 'CT-033', 'CT-034', 'CT-035', 'CT-036', 'CT-037', 'CT-038', 'CT-039', 'CT-040',
    ...
    for ($index = 0; $index -lt $stepItems.Count; $index++) {
        ...
        if ($expectedStepId -in @('port-contract-debug', 'port-contract-release')) {
            ...
            Confirm-Check (
                (Get-JsonIntegerField -Fields $stepFields -Name 'assertions' -Id 'summary-contract-assertions') -eq 601) `
