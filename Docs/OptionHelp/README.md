...
A stanza in an encoder file must match an option of that encoder or of an encoder that inherits from it, so a base file may carry text for a switch only a variant declares. Run the unscoped check after editing a variant file: the base's orphan rule and counters depend on the variant's declarations, and a scoped run reports the counters of the named encoder alone (it applies the parse, link and resource rules to every file in the chain and to the shared files; the orphan and value rules stay with the named encoder).
