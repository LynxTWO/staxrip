...
3. **Fixed on this branch.** Dead code: `Replace(Environment.NewLine, "")` in `OverridingTargetFileName` (`SvtAv1Enc.vb`) ran after `Macro.ExpandParamValues` (`Source/General/Macro.vb`) had already turned CR and LF into `-`. The call is removed and a comment names the macro's replacement; `staxrip.target-file-name` ("line breaks ... become `-`") described the macro's work and stays as it is.
...
6. **Fixed on this branch.** `HbdMds` was off by one (`SvtAv1Enc.vb`): `IntegerValue` emitted the dropdown index while the labels start at -1, so "0: Forces 8-bit" sent `--hbd-mds 1`, "1: Forces 10-bit" sent 2, and "2: 8/10-bit Hybrid" sent 3, always refused. `.Values = {"-1", "0", "1", "2"}` now supplies the emitted strings and `IntegerValue` is removed (it would otherwise still steer `ImportCommandLine` to match by index); the dropdown indices, and therefore saved profiles, are unchanged, and the `Values` of `svt-av1.hbd-mds` describe the fixed behaviour. The b2 crash of `--hbd-mds 1` and `2` on 10-bit input above `--lp 3` still stands and stays in the stanza.
7. **Not a defect; this line is the correction.** Passes 2 and 3 send `p.VideoBitrate` rather than the dialog's Target Bitrate - `sb.Append(" --tbr " & If(pass = 1, TargetBitrate.Value, p.VideoBitrate))` in `SvtAv1Enc.vb` - which reads like an oversight and is the codebase convention: `x265Enc.vb` 1877 and `x264Enc.vb` 1355 are character for character the same expression, and `AOMEnc`, `QSVEnc`, `VCEEnc`, `VvencffappEnc` and the four SVT-AV1 variants all write it too. `p.VideoBitrate` is the project bitrate, and the project owns it: the main window's bitrate box writes it and pushes it into the encoder (`MainForm.vb` 4718), and the target-size search drives it while it hunts for the size you asked for (`GlobalClass.vb` 1135 and 1144). A later pass therefore encodes at the bitrate the project is aiming for, which is the point of a multi-pass encode; making passes 2 and 3 read the dialog box instead would break target-size and Comp. Check encodes for every encoder in StaxRip. **The code is unchanged.** `svt-av1.tbr` already says it correctly ("A value typed here is used by a single pass or the first pass; later passes take the main window's bitrate") and is unchanged as well.
8. **Fixed on this branch.** `--cqp` was formatted with `ToString("0.##")` and no invariant culture (`SvtAv1Enc.vb`), unlike `--crf` and `--qp`; a comma-decimal locale would emit `--cqp 30,25`. The format call now passes `CultureInfo.InvariantCulture` like its two neighbours. Not stated in a stanza (code review, not tested with a locale change).
9. **Fixed on this branch.** `QmMax` was always visible: its `VisibleFunc` tested `EnableQm.Visible` where `QmMin` tests `EnableQm.Value`, so `--qm-max` was sent with matrices off (harmless in a test; the encoder ignores it). The `VisibleFunc` now tests `EnableQm.Value`, matching `QmMin`, and `svt-av1.qm-max` says the control is shown and sent only with matrices on.
...
11. **Fixed on this branch.** `PassesCBR` offered 2-pass and 3-pass that can never work: CBR multi-pass is refused in low delay, CBR itself in random access, and `--pass 3` is outside the encoder's range. `OnValueChanged` now hides both entries with `ShowOption` and resets a stored multi-pass value to 1-pass, keeping the list length so saved profiles are not re-mapped; `svt-av1.pass` says the Constant Bitrate list holds 1-pass only. **The trap here is worth knowing:** `PassesCBR` and `PassesVBR` set neither `.Name` nor `.Switch`, so `CommandLineParam.GetKey` fell through to `Text + HelpSwitch` and both persisted to one entry, `"Passes--pass"` — the first version of this fix reset that shared entry and silently downgraded a saved *Variable Bitrate* 2-pass to 1-pass on every dialog open, and through `SetMetaData` → `ImportCommandLine` with no dialog at all. `PassesCBR` now carries `.Name = "PassesCBR"`, the same disambiguation this file already applies to `PipingToolAVS`/`PipingToolVS` and `CompCheckQuant`/`CompCheckAimedQuality`, so the two modes persist separately and the reset touches only CBR. `KeyInt`/`KeyIntCrf` share `"--keyint"` the same way, deliberately — they mirror each other through `ValueChangedAction` and are one logical setting — so the a13 reset in VBR also clears the Quality-mode "infinite" choice; that is intended, since the entry it clears is the one that cannot work.
12. **Fixed on this branch.** Startup Mini-GOP Size was shown in every rate-control mode but stops VBR and CBR encodes ("only supports CRF/CQP"). A `VisibleFunc` now offers the control in Quality mode only, and `GetArgs` skips invisible params, so a stored value is no longer sent with a bitrate target; `svt-av1.startup-mg-size` (Used when) says the control is hidden there.
13. **Fixed on this branch.** The keyint list's "0: infinite" entry is refused by VBR ("intra period must be > 0"): no file is written and the exit code is 0, so StaxRip reports success. `OnValueChanged` now hides the entry while Variable Bitrate is selected, resetting a stored 0 to the default, and shows it again in the other modes (Constant Bitrate ran with it, tested); `svt-av1.keyint` (value `0`) says the list hides it there.
...
15. **Fixed on this branch.** Chroma-position import: `SetMetaData` passed MediaInfo's H.264/HEVC value (0 left, 1 center, 2 top-left) straight into AV1's list (1 vertical/left, 2 colocated/top-left), so a left source stayed Unknown and a center source was tagged left. A `Select Case` now maps `0 -> 1` and `2 -> 2` and sends nothing for anything else, because AV1 has no value for center; the digits are read with `Regex.Match(..., "\d+")` as a19 reads MaxCLL, so a source whose two fields differ ("Type 0 / Type 1") no longer yields `01`. The evidence that the scales are what they are: MediaInfo fills this field as `"Type " & chroma_sample_loc_type` for H.264 and HEVC and converts AV1 into the same scale (MediaInfoLib `File_Avc.cpp`, `File_Hevc.cpp`, `File_Av1.cpp`, whose table maps AV1's 1 to "Type 0" and 2 to "Type 2" - the mapping in reverse), and the AV1 side is `AV1_CSP_VERTICAL = 1`, `AV1_CSP_COLOCATED = 2` with the encoder's own option text. `svt-av1.chroma-sample-position` states what is filled now.
16. **Fixed on this branch.** The Master Display box was passed verbatim as `--mastering-display`, and a value the bundled build cannot read leaves it spinning at 100% of a core forever, before it opens the input, with no message in StaxRip (b9). `BeforeEncoding` - the hook `x265Enc`, `NVEnc`, `QSVEnc` and `VCEEnc` already use to check a setting before the encoder starts, called for the ordinary and the chunked path alike - and `RunCompCheck`, which starts the encoder of its own, now call `VerifyMasteringDisplay`; it throws `ErrorAbortException`, StaxRip's own "name the problem, stop the job, write it into the log" mechanism, naming the value, the shape and the fix. The check is derived from the encoder's own parser rather than invented: `svt_aom_parse_mastering_display` (SVT-AV1 `Source/Lib/Globals/metadata_handle.c`) walks the string letter by letter and the `default` branch of its switch never advances the pointer, which is the whole of b9, so the check admits exactly what that scanner consumes end to end - `G`, `B`, `R` or `L` and `(number,number)`, or `W` plus one character (it never checks the `P`) and `(number,number)`, in any order, any number of times, either case, with the blanks `strtod` skips. A differential run of the regex against a faithful model of that scanner over 3201 mutations of the reference string admitted nothing that would hang or half-parse, and refused only hexadecimal, `inf` and `nan` literals, which no mastering display needs. The Custom box can still reach the hang, and the four SVT-AV1 variants keep their own copy of `SetMetaData` and are unguarded. `svt-av1.mastering-display` says StaxRip stops the job now; encoder defect b9 stands.
17. **Fixed on this branch.** Typo "2: Aadaptive" in `EnableTF.Options` (`SvtAv1Enc.vb`); the dialog showed it while the stanza's note says "Adaptive". Corrected to "2: Adaptive" here and in the `SvtAv1HdrEnc`, `SvtAv1PsyexEnc` and `SvtAv1TritiumEnc` copies of the list (option text only, no index change); `svt-av1.enable-tf` already said "Adaptive" and is unchanged.
18. **Fixed on this branch,** the half of it that is StaxRip's. Each of the three `masteringDisplay_ColorPrimaries` branches appended `" --color-range 0"` after its `--mastering-display` string, and since `ImportCommandLine` parses the whole accumulated string the later switch won, so a source MediaInfo reported as `Full` came out tagged Studio. The three appends are removed and a comment says why: the primaries of the display a video was graded on say nothing about the video's range. The history holds no reason to keep them - all three arrived with the block itself (`a1cb44a8`, "SvtAv1EncApp: Add VUI metadata import", 2024-05-21), copied from the x265 HDR10 block, where `--range limited` is one of six flags set together with `--output-depth 10`, `--hdr10`, `--repeat-headers`, `--hrd` and `--aud`; only the range append survived the port into SVT-AV1, which has none of the rest. `x264Enc`, `x265Enc`, `NVEnc`, `QSVEnc` and `VCEEnc` still carry the same append, outside this change. The other half is not StaxRip's: `SetMetaData` sends `--content-light` whenever either number is non-zero and the encoder then drops a FALL-only pair, which is encoder defect b10 and stands. `svt-av1.color-range` and `svt-av1.mastering-display` no longer say Studio is set along with Master Display; `svt-av1.content-light.max-fall` is unchanged.
19. **Fixed on this branch.** An AV1 source carrying HDR metadata in both bitstream and container makes MediaInfo report MaxCLL as `1000 / 1000`, which `ToInt` read as 0, so no `--content-light` was sent. `SetMetaData` now takes the first run of digits from the MaxCLL and the MaxFALL strings (the pair is joined the same way), and a value with no digits still yields 0 rather than throwing; the stanzas' "type them only if missing" stays right.
20. **Fixed on this branch.** The third pass was labelled "Video encoding second pass" (`SvtAv1Enc.vb`); it now reads "Video encoding third pass". Not stated; cosmetic.
21. **Fixed on this branch.** The legacy `.Help` strings on `CompCheck` and `CompCheckAimedQuality` (`SvtAv1Enc.vb`; the second said "to adjusts") were held until the stanzas had been read on screen; the maintainer's tone review (section 2, 2026-08-27) covered `staxrip.comp-check` and `staxrip.aimed-quality`, so the strings are removed and the reviewed stanzas are the single source. With a resolved stanza the label help comes from `ShowOptionHelp`, and without one `CommandLineForm` still builds the Minimum/Maximum lines, so the fallback label help stays correct rather than empty.
...
- Maintainer decisions recorded above: whether `svt-av1.rc` or `svt-av1.tbr` should mention the VBR run-to-run variance (b14).
