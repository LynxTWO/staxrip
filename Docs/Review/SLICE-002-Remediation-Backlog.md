...
- 2026-08-26, second sweep - The five designs the maintainer ratified as D-055 through D-059 were implemented, each proven by a red run first, and the attested sweep passed end to end at head ee2db916: port-verify 1503 with 61 contract cases and 747 assertions in each configuration, port-http-windows 5216, port-linux-sandbox 27 with zero runtime writes, port-browser 755, port-inspection 99 executed with 68 published, and the audit 66,098 checks in 1461.4 seconds, writing evidence-audit.json with SHA-256 3d385558db27e885382dcffef5bc5d9e5b1f29a9c14a8145f314f9be67b18caf and its sidecar. Seven mutations were run against the corpus; two survived the first round and both were missing tests rather than equivalent mutants, one because the swap test used a technique the share mode does not govern and one because a shutdown bound sat above the fallback latency, and both tests were strengthened until the mutations were caught. The sweep itself refused the work four times before certifying it, each refusal a law doing its job: two new boundary crossings had to be sanctioned by name, three copies of the reviewed case baseline and three count pins had to move, and a published activation reason collided with the wire vocabulary the shell and the sandbox pin, so the verdict stays in process. This entry postdates the audited set by construction; the audited state is the commit named above. Not proven by execution: the Linux branch of the identity binding, recorded in R-S2-050.
