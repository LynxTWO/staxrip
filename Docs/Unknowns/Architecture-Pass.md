...
- **Notes 2026-09-06, traced.** The end-to-end path is: page fetch, first matching `href`, relative-link resolution against the page host, `WebClient` download to the Desktop with a browser user agent and Referer, 7-Zip extraction, presence check for `Package.Filename`, user-confirmed delete of the current files to the Recycle Bin, user-confirmed copy. Deterministic counts: 160 `DownloadURL` entries, 158 HTTPS, 2 plain HTTP (`Package.vb:87`, `Package.vb:1204`); no hash, signature, host, or size check anywhere in `Source`. The smallest safe edits are written up as the "Tool download integrity" packet in `Docs/Review/Approval-Packets.md` and as finding F-002 in the calibration findings ledger. The owner approved the packet on 2026-09-06 and the three edits landed as fork commit `a512ea89` on `agent/tool-download-integrity`, verified by a standalone harness (17 cases) and both x64 solution builds. The entry stays open for the upstream contract: the fork now refuses plain HTTP, confirms cross-host links, and shows the archive digest, but no package declares a publisher checksum and redirects after the checked link are not re-verified.
