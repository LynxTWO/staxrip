...
- **Owed, and not done:** there is no regression guard. Nothing fails if the header is removed again, so the fix is protected only by the intermittent failure eventually returning, which is exactly the weak position this repository's discipline exists to avoid. A gate assertion naming this header on a body-refusal response should be added, and it is the one piece of this work that is genuinely unfinished rather than deferred by choice.
