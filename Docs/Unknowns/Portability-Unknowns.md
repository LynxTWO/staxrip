...
### P-016: Settings location is resolved interactively and stored in the Windows registry

- **Area or file:** `Source/General/General.vb:140-175`, `Folder.Settings`
- **Concern:** The settings directory is discovered through a modal dialog on first run and remembered in the Windows registry, keyed by the startup directory. Three separate things there do not port, and one of them hangs any automated caller.
- **Why it matters:** A headless engine, a build agent, or a test harness that touches this path does not fail, it **blocks on a dialog no one can see**. The registry key has no cross-platform equivalent. And the cancel path is `Process.GetCurrentProcess().Kill()`, so a caller that dismisses the prompt does not get an exception it can handle, it gets a dead process.
- **Evidence found so far:** Found by accident on 2026-08-22, not by looking. A harness that pointed `Folder.Startup` at an app tree in order to reach `Package` popped StaxRip's settings dialog on the maintainer's desktop and hung for five minutes until it was killed. The path, read afterward: `:143-146` enumerates registry value names under `Software\StaxRip\SettingsLocation` and deletes any whose directory is missing; `:149` reads the location keyed by `Startup`, defaulting to `Startup` itself; `:151-160` shows a four-option `TaskDialog` when that directory does not exist; `:171` kills the process when the dialog returns empty. `Package`'s type initializer reaches this path, which is why merely loading that class in a foreign host is enough to trigger it.
- **Confidence:** verified by execution, unintentionally, and confirmed by reading
- **Likely owner:** Persistence-slice maintainer, jointly with P-002
- **Next best check:** Separate the two questions before designing. Where settings live is a portability question with a settled answer shape: a well-known per-user path, `%APPDATA%` on Windows and the XDG base directory on Linux, resolved without a prompt and without the registry. Whether settings are recoverable is a different question, and the maintainer's instinct on 2026-08-22 that they should be backed up off site is right in substance. It should **not** be implemented by pointing the live store at a synchronised folder: the five on-disk formats are non-atomic `BinaryFormatter` writes over `FileMode.Create`, a sync client copying one mid-write yields a truncated blob that does not deserialize, and `Jobs.dat` and the profile files are guarded by a named mutex that means nothing between two machines. The safe shape is a local live store plus an explicit export that writes a consistent snapshot wherever the user wants it, cloud storage included.
- **Risk level:** high
- **Status:** open
- **Notes:** Recorded 2026-08-22. This is a blocker distinct from P-002's format question and does not wait on it, though both get easier in the same order: atomic writes and a promptless location are far simpler once the formats are not `BinaryFormatter`, which is the sequencing D-053 already argues for. Practical note for anyone building a harness against the legacy assembly: `StaxRip.Folder` and `StaxRip.ObjectHelp` initialize cleanly in a foreign host, `StaxRip.Package` does not, and pointing `Folder.StartupValue` at a real installation is exactly the wrong way to fix that.
