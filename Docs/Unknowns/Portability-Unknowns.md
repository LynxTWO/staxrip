...
- **Confidence:** verified as a reproducible order dependency; one candidate mechanism refuted; the actual mechanism unknown
- **Likely owner:** Cross-platform server and harness maintainer
- **Next best check:** Retain the server's own stderr on the failing path first. The packet records the client's view of a closed connection and says nothing about what the server did, and no amount of reasoning from the client side will settle whether the server closed it. With that in hand, the two surviving candidates are load rather than freshness, since the refuting experiment rebuilt one project in 13 seconds where the wrapper performs a full build plus a self-contained Linux publish of 344 files and 110 MB immediately beforehand; and residue from the wrapper, meaning socket reuse pressure from its own test servers or shared temp and task state under `CrossPlatform/artifacts`.
- **Risk level:** medium
- **Status:** open
- **Notes:** Recorded 2026-08-22. Do not resolve this with a delay or a retry between gates. A pause would remove the symptom without establishing whether the server drops connections under load, which is the question. A delay is acceptable as a diagnostic to confirm the load hypothesis, and not as the fix. The sweep's evidence set was closed by running the gate standalone at the same head, which is honest about what was executed and is not equivalent to the gate passing in sequence. Separately observed and not drift: this gate's check count follows its environment rather than being pinned, at 5182 and 5184 across runs, the same property the browser gate has. Separately verified while gathering this: the application recycles its own process between jobs once private memory exceeds 1500 MB, relaunching itself with `-StartJobs -NoFocus` and closing the running instance (`Source/General/GlobalClass.vb:516-519`). That is on the normal batch path, so any throughput measurement over a long queue has to account for a full application restart mid-run, and a harness that assumes one continuous process will mis-attribute the gap. Findings adjacent to this unknown are in `Docs/Review/Legacy-Efficiency-Findings.md`.
