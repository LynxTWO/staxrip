...
- **What is actually known:** the failure is intermittent, at roughly 8 failures in 18 observed runs, with no identified trigger. It is not random noise either, since the signature is identical every time and the teardown is always clean. No sequence of preceding actions has been shown to cause or prevent it.
...
- **Confidence:** verified as an intermittent failure with an identical signature. Nine candidate mechanisms refuted by measurement, two of which were my own confidently stated causes. Root cause unknown, and black-box elimination from the client side has stopped producing results.
