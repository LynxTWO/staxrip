...
- **Reproduction, reliable:** run `Verify.ps1`, then `Verify-Http.ps1 -Configuration Release`. It fails. Run the HTTP gate alone on a machine that has not recently run the wrapper and it passes. Confirmed 6 failures in 6 attempts following the wrapper, against 7 passes in 7 attempts without it.
...
- **What is established:** the failure is caused by the wrapper specifically; it persists across immediate retries, so it is not a one-shot race; and it decays, with attempts minutes later passing without any intervening action. Timing places it near the end of the gate rather than early: a warm passing run takes about 1950 ms and the failure took 2286 ms, which is longer than a whole passing run. The gate starts two server instances, and the failure signature carries `registeredProcessCount` of 2, so the late scenarios that involve a second instance are where to look first.
- **Confidence:** verified as a reproducible, decaying, wrapper-induced failure. Seven candidate mechanisms refuted by measurement. Root cause unknown.
- **Likely owner:** Cross-platform server and harness maintainer
- **Next best check:** The client-side view is exhausted; every remaining question is about what the server did. The gate records `check=http-send`, which it sets before every request, so the packet cannot say which request died. Two instrumentation steps, in order: give each case an identifier that survives into the failure packet, and retain the server's own stderr on the failing path. Those two facts together should name the scenario in one run. Do this in a working copy rather than in the gate, so the change that diagnoses the gate is not also the change the gate grades.
- **Risk level:** medium
- **Status:** open, substantially narrowed 2026-08-22
- **Notes:** Recorded 2026-08-22. Do not resolve this with a delay or a retry between gates. Now that the failure is known to decay, a pause would reliably hide it, which makes a pause the most tempting and least honest fix available: the gate would go green while whatever the wrapper does to the server remains undiagnosed. A delay is legitimate as a diagnostic, and it was used as one to establish the decay. Separately observed and not drift: this gate's check count follows its environment rather than being pinned, at 5182 and 5184 across runs, the same property the browser gate has.
  ...
  **Consequence for attestation, stated plainly.** A full seven-gate sweep in the documented order cannot pass on this machine while this is open, because the order runs the wrapper immediately before the HTTP gate and that combination fails every time. Green evidence for this head was therefore produced by running the gates individually, with the HTTP gate run when the machine was not in the post-wrapper state. That is an accurate record of what was executed and it is weaker than an in-order sweep, because it cannot show the gates passing as a set in one pass. Any release decision using this evidence should know that. Separately verified while gathering this: the application recycles its own process between jobs once private memory exceeds 1500 MB, relaunching itself with `-StartJobs -NoFocus` and closing the running instance (`Source/General/GlobalClass.vb:516-519`). That is on the normal batch path, so any throughput measurement over a long queue has to account for a full application restart mid-run, and a harness that assumes one continuous process will mis-attribute the gap. Findings adjacent to this unknown are in `Docs/Review/Legacy-Efficiency-Findings.md`.
