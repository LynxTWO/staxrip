...
- **What this makes it:** a defect in how a rejected chunked request terminates, not a flaky test and not an environment problem. A client that sends a body the server refuses should receive a status, and a browser doing the same would see the same reset. Whether the fix belongs in the request law, in the host configuration that governs how a refused body is drained, or in the gate's expectation for this one case, is a design question that has not been settled and touches the loopback request law, so it is approval-gated.
...
- **Confidence:** cause verified by instrumentation and reproduced twice at the same request. The remedy is undecided.
- **Likely owner:** Cross-platform server and harness maintainer
- **Next best check:** Decide where the fix belongs before writing one. Establish first what the server actually does with a refused chunked body, by sending that one request in isolation in a loop and recording the distribution of outcomes; a reset rate under repetition will say whether the response is ever delivered cleanly. Then choose between making the refusal deterministic, which is a request-law change and approval-gated, and accepting a reset as a valid outcome for this case in the gate, which weakens an assertion and should not be done merely to get a green run. The gate's own expectation for this case should be read before either, because it may already permit a reset on some paths.
- **Risk level:** medium
- **Status:** open. Cause located 2026-08-22; the remedy is a separate decision.
